            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><img src="http://172.26.43.150:8080/order/image/{{ $order['proof_of_payment'] }}"
                        ...
                        <td>
                            <form id="action-form-{{ $order['id'] }}" action="{{route('admin.approve', $order['id'])}}"
                                method="POST">
                                @csrf
                                @method('PUT')
                                <button class="btn btn-sm btn-primary">Approve</button>
                            </form>
                            <form id="action-form-{{ $order['id'] }}" action="{{route('admin.reject', $order['id'])}}"
                                method="POST">
                                @csrf
                                @method('PUT')
