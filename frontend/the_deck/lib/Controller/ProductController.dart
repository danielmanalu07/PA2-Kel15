...
class ProductController extends GetxController {
  ...
  Future<void> fetchProductList() async {
    final response =
        await http.get(Uri.parse('http://192.168.66.215:8080/product'));
  ...
  Future<void> getProductById(int id) async {
    final response =
        await http.get(Uri.parse('http://192.168.66.215:8080/product/${id}'));
  ...
  Future<void> getProductByCategoryId(int categoryId) async {
    final response = await http.get(
        Uri.parse('http://192.168.66.215:8080/product/category/$categoryId'));
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body)['message'] ?? [];
      productList.value = data.map((json) => Product.fromJson(json)).toList();
    } else {
